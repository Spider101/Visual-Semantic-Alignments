...
from config.resources import vist_data_path
...
#-----------------------------------------------------------------------------#
path_to_models = join(vist_data_path, "skipthoughts", "")
path_to_tables = join(vist_data_path, "skipthoughts", "")
#-----------------------------------------------------------------------------#
...
class Encoder(object):
    ...
    def encode(self, X, use_norm=True, verbose=False, batch_size=128, use_eos=False):
      """
...
def encode(model, X, use_norm=True, verbose=True, batch_size=128, use_eos=False):
    ...
    for k in ds.keys():
        if verbose:
            print(k)
        numbatches = int(len(ds[k]) / batch_size + 1) #hack to make sure num batches is an integer
...
def preprocess(text):
    ...
    #hack to catch strings with just whitespace
    for idx in range(len(text)):
        if text[idx].replace(" ", "") == "":
            text[idx] = "\b" #replace an identifier for whitespace
...
def init_tparams(params):
    ...
    #hack to avoid code breaking down in py3
    try:
        iterate = dict.iteritems
    except:
        iterate = dict.items
    
    for kk, pp in iterate(params):
